...
`/commit` や `/deps-update` は実作業を ops-runner に委譲するが，**本スキルは主要な処理をメインループで実行する**．理由は以下の 3 点であり，いずれも**権限設定では解消しない本質的な理由**である．

1. **確認フローで往復が 2 回に割れる**: ステップ 3 のユーザー最終確認はメインループに残らざるを得ないため，委譲すると「委譲① 判定 → メインで確認 → 委譲② 実行・監視」となる．サブエージェントはコンテキストを引き継がないので，本手順書（約 200 行）の読み込みと MCP スキーマの遅延ロードが**2 回ずつ**発生する．一方で節約できるのは `list_deploys` の返り値（数千トークン）だけで，収支はマイナスになる
2. **委譲できる機械的な部分が残らない**: デプロイ要否の判定もビルドログの読解もどちらも**判断**であり，ops-runner の行動規範（「判断しない」）に反する．本プロジェクトの方針は「判断はメイン，機械的操作は ops-runner」である
3. **ツール許可はスキル単位で切り替わらない**: ops-runner に `mcp__render__*` を許すと，`/commit`・`/task-*`・`/deps-update` から呼ばれる ops-runner も `trigger_deploy` を撃てる構造になる．影響が本スキルに閉じない

※ 「ops-runner の許可ツールに `mcp__render__*` が無い」ことは委譲しない理由**ではない**（権限を追加すれば外せる可変の制約であるため）．上記 3 点が本質的な理由である．
※ ビルド失敗が頻発してメインのコンテキストがログで膨れるようなら，そのときは「ログ取得・要約専用の**読み取り専用**サブエージェント（`list_logs` / `get_deploy` のみ許可）」の切り出しを検討する．`trigger_deploy` は委譲しない
