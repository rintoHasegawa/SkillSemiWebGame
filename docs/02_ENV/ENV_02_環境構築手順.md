...
3. 動作確認コマンドを実行する．
   ...
   ※ pnpm 自体が起動しない場合（`ERR_UNKNOWN_BUILTIN_MODULE: node:sqlite` 等）: Node 20 に対応しない新しい pnpm がグローバルに入っている．pnpm の版は `package.json` の `packageManager`（`pnpm@10.28.2`）に固定しており，`.devcontainer/devcontainer.json` の node feature の `pnpm` も同じ版を指定している．`postcreate.sh` が起動時に版を揃えるが，手動で直す場合は `npm i -g pnpm@10.28.2` を実行する（応急処置は `corepack pnpm <コマンド>`）．`packageManager` の版を上げるときは `devcontainer.json` の `pnpm` も必ず同じ版に更新すること．
