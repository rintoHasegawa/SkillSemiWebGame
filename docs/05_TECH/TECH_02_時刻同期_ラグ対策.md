...
`GAME_START`受信時に，PONGを待たずに`serverNow`からオフセットの初期値を設定する．

- 算出: `offsetMs` = `serverNow` - `GAME_START`受信時刻
- この値は片道遅延を補正していないため，真のオフセットより**片道遅延ぶん過小**になる（PONGサンプルは`estimatedOneWayMs`で補正済みのため，両者の定義は片道遅延ぶん食い違う）
- そのためシード値は**暫定値**として扱い，最初の有効なPONGサンプルで実測値に置換する（「棄却の例外と復帰」を参照）
- 実測サンプルを取り込み済みの場合，シードは測定結果を上書きしない
- シードから最初のPONG取り込みまで（既定3000ms）は，片道遅延ぶんのズレが残る
...
- しきい値: 250ms（`MAX_ACCEPTED_OFFSET_JUMP_MS`）
- 前回のオフセットとの差が250msを超えるサンプルは棄却する
- Wi-Fi切り替えやネットワーク経路変更による急激な変動を吸収する
- 棄却したサンプルはRTTの平滑化にも反映しない

### 棄却の例外と復帰 (Rejection Exceptions & Recovery)

棄却のみでは，基準となるオフセットが誤った値で固定されたときに復帰できない．そのため以下の2つの例外を設ける．

- **暫定シード直後**: シード値は片道遅延ぶん過小なので，最初の有効サンプルは棄却判定を行わず実測値で**置換**する．この例外が無いと，片道遅延が250msを超える環境（RTT 500ms超）では正しいサンプルが全て外れ値と判定され，オフセットが恒久的にロックする
- **連続棄却の上限**: 連続棄却が`MAX_CONSECUTIVE_REJECTED_SAMPLES`（3回）に達した後のサンプルは棄却せず，実測値でベースラインを組み直す．サンプルを採用した時点で連続棄却の記録は0に戻る

いずれの例外でもEWMA混合ではなく置換とする．混合にすると残差が`(1 - 0.12) × 片道遅延`として残り，片道遅延が約284msを超える環境で再びロックするため．

RTTが`MAX_ACCEPTED_RTT_MS`（1000ms）を超えるサンプルはRTT外れ値フィルタで先に落ちるため，連続棄却には数えられない．
